...
/**
 * Wrapper around the result from an interceptor.
 * 
 * Note: interceptors are identified by instance reference, rather than just type.
 * This is important as, in the future, there may be only a small number of interceptor types,
 * with many different instances constructed through learning.
 * @author Malcolm Lett
 */
public class InputInterceptorResult {
	// interceptor which produced the emittedEvents
	private final InputInterceptor interceptor;
	
	// events generated by processor
	private final List<Event> emittedEvents;

	public InputInterceptorResult(InputInterceptor interceptor, Event emittedEvent) {
		this.interceptor = interceptor;
		this.emittedEvents = (emittedEvent == null) ? Collections.emptyList() :  Collections.singletonList(emittedEvent);
	}

	public InputInterceptorResult(InputInterceptor interceptor, List<Event> emittedEvents) {
		this.interceptor = interceptor;
	...
	public String toString() {
		if (emittedEvents.isEmpty()) {
			return String.format("%s:{0}", interceptor.getClass().getSimpleName());
		}
		else {
			return String.format("%s:{%d @ %f => %s}", interceptor.getClass().getSimpleName(),
				emittedEvents.size(),
